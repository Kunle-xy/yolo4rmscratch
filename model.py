...
class CNNBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, bn_act = True, **kwargs):
        super().__init__()
...
class ScalePrediction(nn.Module):
    def __init__(self, in_channel, num_classes):
        ...
        self.pred = nn.Sequential(
            ...
            CNNBlock(2*in_channel, (num_classes + 5) *3, bn_act=False, kernel_size = 1)
        )
    ...
    def forward(self, x):
        return(
            self.pred(x)
            .reshape(x.shape[0], 3, 
            self.num_classes + 5, x.shape[2], x.shape[3]).permute(0,1,3,4, 2)
        )
